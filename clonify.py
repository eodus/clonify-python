...
def analyze_collection(coll, mongo):
    '''
    ...
    if mongo:
        ...
        seqs = get_seqs_cdr_labeller(coll)
    ...
    if mongo:
        ...
        cluster_stats = update_db_fake(clusters)
    print_update_end()
    updateEnd = time.time()
    ...
    if args.output != '':
        print_write_start()
        coll_name = coll if mongo else "cdr_labeller"
        write_output(args.output, coll_name, clusters)
        print_write_end()
    else:
        print_write_null()
...
def main():
    if args.input is None:
        for c in get_collections():
            analyze_collection(c, mongo=True)
    else:
        print "Analyze cdr_labeller output " + args.input
        analyze_collection(args.input, mongo=False)
